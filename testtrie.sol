    ...
    function verifyERC721Balance(
        ...
        bytes32 storageKey = keccak256(
            abi.encode(
                ...
                uint256(2) // The _owners mapping is at the third slot in the layout.
            )
        );
        //Verify the balance for the user address is correct
        require(
            SecureMerkleTrie.verifyInclusionProof(
                abi.encodePacked(storageKey), RLPWriter.writeAddress(user), storageProof, storageRoot
            )
        );
        return true;
    }
